...
class MapsService {
   ...
   /**
     ...
     */
    public function buildingExtent($public_id, $value) {


            $val1 = "building_info.buildings";
             // Get the minimum X coordinate of the building extent
            $xmin = array_pluck(DB::select(DB::raw("select st_xmin(ST_Extent(geom)) from building_info.buildings where " . $public_id . " = '" . $value . "'")), 'st_xmin')[0];
             // Get the minimum Y coordinate of the building extent
            $ymin = array_pluck(DB::select(DB::raw("select st_ymin(ST_Extent(geom)) from building_info.buildings where " . $public_id . " = '" . $value . "'")), 'st_ymin')[0];
             // Get the maximum X coordinate of the building extent
            $xmax = array_pluck(DB::select(DB::raw("select st_xmax(ST_Extent(geom)) from building_info.buildings where " . $public_id . " = '" . $value . "'")), 'st_xmax')[0];
             // Get the maximum Y coordinate of the building extent
            $ymax = array_pluck(DB::select(DB::raw("select st_ymax(ST_Extent(geom)) from building_info.buildings where " . $public_id . " = '" . $value . "'")), 'st_ymax')[0];

            // Get the latitude and longitude of the centroid of the building
            $lat = array_pluck(DB::select(DB::raw("select ST_Y (ST_Transform (ST_Centroid(geom), 4326)) as lat from building_info.buildings where " . $public_id . " = '" . $value . "'")), 'lat')[0];
            $long = array_pluck(DB::select(DB::raw("select ST_X (ST_Transform (ST_Centroid(geom), 4326)) as long from building_info.buildings where " . $public_id . " = '" . $value . "'")), 'long')[0];
